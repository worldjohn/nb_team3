...
head(sales.sport)
vif1.model = lm(LOG_SALES~ITEM_CNT+PRICE+MAXTEMP+RAIN_DAY+
                   HOLIDAY+DUSTDAY+MONTH+Quart, data=sales.sport)
vif(vif1.model)
# ITEM_CNT     PRICE   MAXTEMP  RAIN_DAY   HOLIDAY   DUSTDAY     MONTH     Quart 
# 2.569790  2.130401  2.120471  2.097128  1.081136  1.265182 20.242438 19.996087
# MONTH와 Quart간의 다중공선성이 보인다.
# 기준은 4이상이면 의심, 10이상이면 제거해야함
# 종속변수와의 상관관계가 낮은 쪽을 제거

# Quart를 제거하고 다시 분석
vif2.model = lm(LOG_SALES~ITEM_CNT+PRICE+MAXTEMP+RAIN_DAY+
                    HOLIDAY+DUSTDAY+MONTH, data=sales.sport)
vif(vif2.model)
# ITEM_CNT    PRICE  MAXTEMP RAIN_DAY  HOLIDAY  DUSTDAY    MONTH 
# 2.569755 2.122715 2.104406 2.071148 1.073535 1.167894 1.296474 
# 다중공선성문제가 해결됨.
